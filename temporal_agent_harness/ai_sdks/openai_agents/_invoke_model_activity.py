...
class StreamingActivityModelInput(ActivityModelInput, total=False):
    ...
    ``str`` with no factory is treated as an external output topic name."""
...
class ModelActivity:
    ...
    def __init__(
        ...
        *,
        ...
        Left ``None``, streaming falls back to publishing raw events to an
        external output topic named by the token.
        """
    ...
    async def invoke_model_activity_streaming(
        ...
        token) republishes raw events to an external output topic so
